...
mytable <- read.table(file="counts_files.merged_data.txt.DESeq_blind.PREPROCESSED.txt.Kruskal-Wallis.hits.project.project_id.1.STATS_REMOVED.filtered.byMedian.txt",row.names=1,header=TRUE,sep="\t",colClasses = "character", check.names=FALSE,comment.char = "", fill=TRUE, blank.lines.skip=FALSE)
p.vector <- c()
for (row in rownames(mytable)) {
  row <- strsplit(row, "\\.")[[1]][1]
  result <- shapiro.test(as.numeric(mytable[row,]))
...
write.table(p.table, file = "shapiro.test.sorted", col.names = F, row.names = F, sep = "\t")
